    ...
    <div class="organizations">
      <%= for organization <- Accounts.get_user_organizations(current_user) do %>
        <%= if current_organization && organization.id != current_organization.id do %>
          <div role="none">
            <a phx-target={@myself} phx-click="default-organization" phx-value-organization_id={organization.id} class="block w-full px-4 py-2 text-sm text-zinc-700 hover:bg-zinc-200 focus:bg-zinc-300" role="menuitem" tabindex="-1" id="options-menu-item-0">
              <span class="flex w-full items-center justify-between">
                <span class="flex min-w-0 items-center justify-between space-x-3">
                  <%= if organization.logo do %>
                    <img src={Logo.url({organization.logo, organization}, :original)} class="h-10 w-10 rounded-lg" />
                  <% else %>
                    <span class="mr-1.5 inline-flex h-10 w-10 items-center justify-center rounded-lg bg-zinc-500">
                  ...
                  <span class="flex min-w-0 flex-1 flex-col">
                    <span class="text-md truncate font-medium text-zinc-900"></span>
