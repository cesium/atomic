defmodule AtomicWeb.Components.Page do
  ...
  slot :title_wrapper, optional: true, doc: "Slot for the title wrapper."
  ...
  def page(assigns) do
    ...
    <div class="flex min-h-full flex-col items-stretch justify-between lg:flex-row">
      <div class={"#{if @bottom_border, do: 'border-b', else: ''} min-h-[100vh] flex w-full flex-col bg-white lg:flex-row lg:border-r"}>
        <main class="relative z-0 lg:z-40 mb-10 flex-1 overflow-y-auto focus:outline-none xl:order-last">
          <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
            <div class="my-6 flex min-w-0 flex-row items-center justify-between">
              <%= render_slot(@title_wrapper, @title) %>
              <h1 :if={@title_wrapper == []} class="flex-1 select-none truncate text-2xl font-bold text-gray-900">
                <%= @title %>
              </h1>
              <%= render_slot(@actions) %>
            </div>
