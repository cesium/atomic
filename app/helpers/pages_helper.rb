module PagesHelper
  def carousel_image_tag(file, data = {}, **args)
    ...
    args[:data]&.reverse_merge!(default_data) || args[:data] = default_data
    image_pack_tag(file, args)
  end

  def slider_link(start_second:, x:, y:, title:, target:)
    content_tag :a, href: target.to_s, target: "_blank" do
      content_tag :div, title.to_s,
  ...
  def partner(image_file:, link:, alt:)
    content_tag(
      ...
      link_to(link, target: "_blank") do
        image_pack_tag(image_file, alt: alt, class: "img-responsive")
      end,
      class: "item",
    )
  end
