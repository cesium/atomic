  ...
  def render(assigns = %{tab: :welcome}) do
    ...
    <div class="lsb-welcome-page">
      <p>
        ...
        Explore the generated <code class="lsb-inline">*.story.exs</code> files in your <code class="inline">/storybook</code> directory. When you're ready to add your own, just drop your new story & index files into the same directory and refresh your storybook.
      </p>
          ...
          <%= for {dt, link} <- @items do %>
            <div class="lsb-py-4 lsb-max-w-full sm:lsb-grid sm:lsb-grid-cols-3 sm:lsb-gap-4 sm:lsb-py-5 sm:lsb-px-6">
              ...
              <dd class="lsb-mt-1 lsb-text-base lsb-text-slate-400 lsb-group lsb-cursor-pointer lsb-max-w-full sm:lsb-col-span-2 sm:lsb-mt-0">
                <a class="lsb-max-w-full lsb-inline-block lsb-truncate group-hover:lsb-text-indigo-700" href={link} target="_blank">
                  <%= link %>
                </a>
              </dd>
            </div>
